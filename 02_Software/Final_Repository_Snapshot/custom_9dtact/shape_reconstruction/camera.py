...
class Camera:
    ...
    def get_raw_image(self):
        if self.cap is None:
            raise RuntimeError(
                "This Camera instance was created with open_camera=False. "
                "Supply images from the ROS subscriber instead."
            )
        success, image = self.cap.read()
        if not success or image is None:
            raise RuntimeError("The tactile camera did not return an image.")
        return image

    def close(self):
        """Release the local camera handle when this instance owns one."""
