...
def calculate_accuracy(predict_wrench, label_wrench):
    denominator = torch.clamp(torch.abs(label_wrench), min=1e-6)
    accuracy = torch.sum(torch.abs(predict_wrench - label_wrench) / denominator, dim=0)
...
class ForceEstimation:
    def __init__(self, cfg):
        ...
        dataset_root = Path(cfg['data_dir']).expanduser().resolve()
        dataset_normalization_metadata = load_dataset_normalization_metadata(
            dataset_root,
            cfg,
            allow_legacy_fallback=bool(
                cfg.get('allow_legacy_dataset_metadata_fallback', True)
            ),
        )
        wrench_min, wrench_max = wrench_limits(cfg)
        self.min_wrench = torch.tensor(wrench_min, dtype=torch.float).to(self.device)
        self.wrench_span = torch.tensor(
            wrench_max - wrench_min, dtype=torch.float
        ).to(self.device)
        ...
        if train_mode:
            ...
            write_inference_metadata(
                self.save_dir, cfg, dataset_metadata=dataset_normalization_metadata
            )
            ...
            metadata_min = vector6(metadata['wrench_min'], 'metadata wrench_min')
            metadata_max = vector6(metadata['wrench_max'], 'metadata wrench_max')
            self.min_wrench = torch.tensor(
                metadata_min, dtype=torch.float
            ).to(self.device)
            self.wrench_span = torch.tensor(
                metadata_max - metadata_min, dtype=torch.float
            ).to(self.device)
