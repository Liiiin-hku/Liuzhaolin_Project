...
class Visualizer:
    ...
    def ros_callback(self, msg):
        ...
        if values.shape != (6,) or not np.all(np.isfinite(values)):
            rospy.logerr_throttle(1.0, "Rejected invalid normalized wrench message")
            return
        self.wrench = np.clip(values, 0.0, 1.0)
...
if __name__ == '__main__':
    ...
    normalized_topic = str(
        rospy.get_param('~predicted_normalized_topic', '/predicted_wrench_normalized')
    )
    predicted_force_sub = rospy.Subscriber(
        normalized_topic, Float64MultiArray, visualizer.ros_callback
    )
